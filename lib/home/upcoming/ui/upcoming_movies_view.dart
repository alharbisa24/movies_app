...
class UpcomingMoviesView extends StatefulWidget {
  ...
  const UpcomingMoviesView({super.key, required this.moviesList});
...
class _UpcomingMoviesViewState extends State<UpcomingMoviesView> {
  ...
  Widget build(BuildContext context) {
        final genresList = context.watch<UpcomingMoviesCubit>().genresList;
  ...
  children: [
     Text(
      AppLocale.upcoming_movies.getString(context), style: AppTextStyles.heading4(color: textColor),
    ),
...
Widget upcomingMovieWidget(BuildContext context, Movie movie, List<Genre> genresList){
      final isDark = Theme.of(context).brightness == Brightness.dark;
    final textColor = isDark ? Colors.white : Colors.black;
